...
from app.crud.crud_reporte_neumatico import crear_reporte_neumatico
...
class FormularioNeumaticoService:
    """
    Capa de Servicio para procesar formularios de neumáticos asociados exclusivamente a usuario_id.
    """
    ...
    async def procesar_formulario(
        ...
        if db is not None:
            try:
                reporte_db = await crear_reporte_neumatico(
                    db=db,
                ...
                print(f"⚠️ Error al guardar en base de datos: {err}")
        ...
        print(f"📌 [CAPA DE SERVICIO]: {resumen_procesamiento}")
